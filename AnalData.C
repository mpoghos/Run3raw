  ...
  for(int iSM = 0; iSM<20; iSM++)
   {
    ...
    for(int iG =0; iG<2; iG++)
	 ...
	 hADC_max[iSM][iG] = new TH1F (Form("hADC_max_SM%02d_%s",iSM, iG==0? "HG": "LG"), Form("hADC_max_SM%002d_%s",iSM, iG==0? "HG": "LG"), 500, -0.5, 499.5);

       }
   }

auto *hBC = new TH1I("hBC","hBC", 3564, -0.5, 3563.5);
auto *hBCvsNChannel = new TH1I("hBCvsNChannel","hBCvsNChannel", 3564, -0.5, 3563.5);
...
//	std::cout << "HW=" << chan.getHardwareAddress() <<  " (FEC"<< chan.getFECIndex() << "): ";
      ...
      if(debug > 1 || kIsCalib)
	std::cout << setw(4) << "SM"<<iSM<<"/DDL"<< setw(2) <<feeID % 2 << "/HW=" << setw(4) << chan.getHardwareAddress() << "/FEC"<<chan.getFECIndex()<<"/CF"<< chantype <<" ("<< setw(2) << col <<","<< setw(2)<<row << "): ";
...
if( ((ADCmax - ADCmin)> 50)  && 1)
{
	...
	std::cout << setw(4) << "SM"<<iSM<<"/FEC"<<chan.getFECIndex()<<"/CF"<< chantype <<" ("<< setw(2) << col <<","<< setw(2)<<row << "): ";
...
TCanvas *cBCvsNChannel = new TCanvas("cBCvsNChannel","cBCvsNChannel",10,410, 400,400);
